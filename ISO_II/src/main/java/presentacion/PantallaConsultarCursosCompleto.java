...
public class PantallaConsultarCursosCompleto extends JFrame {
	...
	public PantallaConsultarCursosCompleto() {
		...
		String [] AllEstados = {"TODOS", "PROPUESTO", "VALIDADO", "PROPUESTA_RECHAZADA", "EN_MATRICULACIÓN", "EN_IMPARTICIÓN", "TERMINADO"};
		...
		panel.add(TiposBox);
		String [] AllEstados1 = {"TODOS", "MASTER", "EXPERTO", "ESPECIALISTA", "FORMACION_AVANZADA", "FORMACION_CONTINUA", "MICROCREDENCIALES", "CORTA_DURACION","VERANO_Y_EXTENSION"};
			...
			public void actionPerformed(ActionEvent e) {
				...
				for(int j=0; j<edicionCursos.length; j++) {
						if (buscar.getText().length()==0 || edicionCursos[j]== Integer.parseInt(buscar.getText()) ){
							if(estadoCursos[j]==EstadosBox.getSelectedItem().toString() || EstadosBox.getSelectedItem().toString()=="TODOS" ) {
								if(TipoCursos[j]==TiposBox.getSelectedItem().toString() || TiposBox.getSelectedItem().toString()=="TODOS" ) {
